...
class AddressBook(UserDict):    
    ...
    def search_records(self, search_phrase: str):
        """search_recrds method of AdressBook class that meets the criteria of searching anywhere in the adress book."""
        results = []
        for name, record in self.data.items():
            if search_phrase in name.value.lower():
                ...
                for phone in record.phones:
                    if search_phrase in phone.number:
                        results.append(record)
                        break
        if not results:
...
def handle_cmd_search(ab_instance: AddressBook):
    search_criteria = input("SEARCH >> Search for any prhase or number: ").strip().lower()
